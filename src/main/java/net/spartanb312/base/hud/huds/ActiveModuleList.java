...
public class ActiveModuleList extends HUDModule {
    ...
    Setting<Color> listColor = setting("ListColor", new Color(new java.awt.Color(100, 61, 255, 200).getRGB()));
    Setting<Boolean> listRainbowRoll = setting("ListRainbowRoll", false).des("Rolling list rainbow").when(() -> listColor.getValue().getRainbow());
    Setting<Float> listRainbowRollSize = setting("ListRainbowRollSize", 1.0f, 0.1f, 2.0f).whenTrue(listRainbowRoll).when(() -> listColor.getValue().getRainbow());
            ...
            public void onUpdate(ScaledResolution resolution, int mouseX, int mouseY) {
                ...
                for (Module module : moduleList) {
                    ...
                    if (listColor.getValue().getRainbow() && listRainbowRoll.getValue()) {
                        color = ColorUtil.rolledRainbow(index * 100, listRainbowRollSize.getValue(), listColor.getValue().getRainbowSpeed(), listColor.getValue().getRainbowSaturation(), listColor.getValue().getRainbowBrightness());
                    }
                    else {
