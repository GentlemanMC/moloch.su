...
import me.afterdarkness.moloch.mixinotherstuff.AccessorInterfaceShaderGroup;
...
public class RenderUtils2D {
    ...
    public static boolean isMouseInRect(int mouseX, int mouseY, float startX, float startY, float endX, float endY) {
        return mouseX >= Math.min(startX, endX) && mouseX <= Math.max(startX, endX) && mouseY >= Math.min(startY, endY) && mouseY <= Math.max(startY, endY);
    }

    public static void betterScissor(float x, float y, float width, float height) {
        ScaledResolution scaledResolution = new ScaledResolution(mc);
        float scale = scaledResolution.getScaleFactor();

        GL11.glScissor((int)(x * scale), (int)(mc.displayHeight - (y * scale) - (height * scale)), (int)(width * scale), (int)(height * scale));
    }
}
