...
public class Panel {
    ...
    public void setup() {
        for (Module m : ModuleManager.getModules()) {
            if (m.category == category && m != ModuleManager.getModule(BackgroundThreadStuff.class)) {
        ...
        if (mc.currentScreen != null) {
            ...
            if (ClickGUI.instance.rectHorns.getValue()) {
                ...
                if (ClickGUI.instance.rectHornsHoverDifColor.getValue()) {
                    ...
                    if (ClickGUI.instance.rectHornsHoverColorSmooth.getValue() && storedHornsHoverLoops.containsKey(category.categoryName) && !isCategoryHovered(mouseX, mouseY)) {
            ...
            if (!elements.isEmpty()) {
                ...
                for (ModuleButton button : elements) {
                    ...
                    if (button.isHovered(mouseX, mouseY, button.x, button.y, button.x + button.width, button.y + button.height) && !button.getDescription().equals("")) {
                        ClickGUIFinal.description = new Pair<>(button.getDescription(), new Vec2I(mouseX, mouseY));
                    }
                    ...
                    for (Component component : visibleSettings) {
                        ...
                        if (component.isHovered(mouseX, mouseY, component.x, component.y, component.x + component.width, component.y + component.height) && !component.getDescription().equals(""))
            ...
            if (isCategoryHovered(mouseX, mouseY) && ClickGUI.instance.categoryRectHoverDifColor.getValue()) {
            ...
            if (ClickGUI.instance.categoryRectHoverDifColor.getValue() && ClickGUI.instance.categoryRectHoverColorSmooth.getValue() && storedCategoryHoverLoops.containsKey(category.categoryName) && !isCategoryHovered(mouseX, mouseY)) {
            ...
            if (isCategoryHovered(mouseX, mouseY) && ClickGUI.instance.categoryRectHoverShadowGradientAlpha.getValue()) {
            ...
            if (ClickGUI.instance.categoryRectHoverShadowGradientAlpha.getValue() && ClickGUI.instance.categoryRectHoverColorSmooth.getValue() && storedCategoryShadowGradientHoverLoops.containsKey(category.categoryName) && !isCategoryHovered(mouseX, mouseY)) {
                int hoverLoops = storedCategoryShadowGradientHoverLoops.get(category.categoryName);
                if (hoverLoops <= 0) {
                    hoverLoops = 0;
                }
            ...
            if (CustomFont.instance.categoryFont.getValue() == CustomFont.FontMode.Minecraft) {
                ...
                if (ClickGUI.instance.categoryRectHoverTextScale.getValue()) {
                    ...
                    if (isCategoryHovered(mouseX, mouseY)) {
                        ...
                        hoverCategoryTextScaleLoops += 0.1f * ClickGUI.instance.categoryRectHoverTextScaleFactorIn.getValue();
                ...
                if (ClickGUI.instance.categoryRectHoverTextScale.getValue() && !isCategoryHovered(mouseX, mouseY)) {
                    float hoverCategoryTextScaleLoops = storedCategoryTextScaleLoops.get(category.categoryName);
    ...
    public void drawShadowsAndGlow(int mouseX, int mouseY) {
        ...
        if (ClickGUI.instance.categoryRectHoverEffect.getValue()) {
            if (ClickGUI.instance.categoryRectHoverParticlesMode.getValue() != ClickGUI.CategoryRectHoverParticlesMode.None) {
                ...
                if ((random.nextFloat() < ClickGUI.instance.categoryRectHoverParticlesGenerateRate.getValue()) && isCategoryHovered(mouseX, mouseY)) {
    ...
    public void onMouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0 && isCategoryHovered(mouseX, mouseY)) {
            x2 = x - mouseX;
            y2 = y - mouseY;
            prevY2 = prevY - mouseY;
            dragging = true;
        ...
        if (isCategoryHovered(mouseX, mouseY) || (extended && isPanelBodyHovered(mouseX, mouseY))) {
            if (category.isHUD) {
                HUDEditorRenderer.panelToSwitch = this;
                HUDEditorRenderer.switchPanelFlag = true;
            }
            else {
                ClickGUIRenderer.panelToSwitch = this;
                ClickGUIRenderer.switchPanelFlag = true;
            }
        }

        if (mouseButton == 1 && isCategoryHovered(mouseX, mouseY)) {
            extended = !extended;
            panelTimer.reset();
        }
    }
    ...
    public boolean isCategoryHovered(int mouseX, int mouseY) {
        ...
        return RenderUtils2D.isMouseInRect(mouseX, mouseY, startX, startY, endX, endY)
                && (!Component.anyExpanded || !RenderUtils2D.isMouseInRect(mouseX, mouseY, Component.expandedX, Component.expandedY, Component.expandedEndX, Component.expandedEndY))
                && isHighestHovered(mouseX, mouseY, this);
    }

    public boolean isPanelBodyHovered(int mouseX, int mouseY) {
        AtomicInteger visibleSettingHeight = new AtomicInteger();
        elements.stream()
                .filter(button -> button.isExtended)
                .forEach(button -> button.settings.stream()
                        .filter(Component::isVisible)
                        .forEach(setting -> visibleSettingHeight.addAndGet(setting.height)));
        float extraY = height + 2 + (((ClickGUI.instance.panelExtensions.getValue() == ClickGUI.PanelExtensions.Top || ClickGUI.instance.panelExtensions.getValue() == ClickGUI.PanelExtensions.Both)) ? ClickGUI.instance.panelExtensionsHeight.getValue() + 2 : 0);
        return RenderUtils2D.isMouseInRect(mouseX, mouseY, x + 5, y + extraY, x + width - 5, y + extraY + height * elements.size() + visibleSettingHeight.get())
                && (!Component.anyExpanded || !RenderUtils2D.isMouseInRect(mouseX, mouseY, Component.expandedX, Component.expandedY, Component.expandedEndX, Component.expandedEndY))
                && isHighestHovered(mouseX, mouseY, this);
    }

    public boolean isAnyCategoryHovered(int mouseX, int mouseY) {
        int startX = (int)(x + ClickGUI.instance.rectX.getValue() - (ClickGUI.instance.rectWidth.getValue() / 2));
        int endX = (int)(x + width + ClickGUI.instance.rectX.getValue() + (ClickGUI.instance.rectWidth.getValue() / 2));
        int startY = (int)(y - (ClickGUI.instance.rectHeight.getValue() / 2) + ClickGUI.instance.rectY.getValue());
        int endY = (int)(y + (height + (ClickGUI.instance.rectHeight.getValue() / 2)) + ClickGUI.instance.rectY.getValue());
    ...
    public boolean isAnyPanelBodyHovered(int mouseX, int mouseY) {
        AtomicInteger visibleSettingHeight = new AtomicInteger();
        elements.stream()
                .filter(button -> button.isExtended)
                .forEach(button -> button.settings.stream()
    ...
    public static boolean isHighestHovered(int mouseX, int mouseY, Panel targetPanel) {
        List<Panel> hoveredPanels;
        if (ModuleManager.getModule(HUDEditor.class).isEnabled()) {
            hoveredPanels = HUDEditorRenderer.instance.panels.stream()
                                .filter(panel -> panel.isAnyCategoryHovered(mouseX, mouseY) || panel.isAnyPanelBodyHovered(mouseX, mouseY))
                                .collect(Collectors.toList());

            return !hoveredPanels.isEmpty() && hoveredPanels.stream()
                    .sorted(Comparator.comparing(panel -> HUDEditorRenderer.instance.panels.indexOf(panel)))
                    .collect(Collectors.toList())
                    .get(0) == targetPanel;
        } else {
            hoveredPanels = ClickGUIRenderer.instance.panels.stream()
                                .filter(panel -> panel.isAnyCategoryHovered(mouseX, mouseY) || panel.isAnyPanelBodyHovered(mouseX, mouseY))
                                .collect(Collectors.toList());

            return !hoveredPanels.isEmpty() && hoveredPanels.stream()
                    .sorted(Comparator.comparing(panel -> ClickGUIRenderer.instance.panels.indexOf(panel)))
                    .collect(Collectors.toList())
                    .get(0) == targetPanel;
        }
    }

    public void scrollPanel(int passedms, int dWheel) {
        if (!Component.shouldScroll || dragging) return;
        ...
        y = (int) MathUtilFuckYou.interpNonLinear(prevY, prevY + ClickGUI.instance.scrollAmount.getValue() * scrollUpOrDown, scrollDelta / 300.0f, ClickGUI.instance.scrollFactor.getValue() / 2.6667f);
    }
}
