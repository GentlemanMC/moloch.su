...
import me.afterdarkness.moloch.gui.components.ColorButton;
import me.afterdarkness.moloch.module.modules.client.CustomFont;
import me.afterdarkness.moloch.gui.components.VisibilityButton;
...
public class ModuleButton extends net.spartanb312.base.gui.Component {
    ...
    private static final HashMap<String, Float> storedLocalLoops = new HashMap<>();
    private static final HashMap<String, Integer> storedDecayFactors = new HashMap<>();


    private static final HashMap<String, Float> storedScaleLoops = new HashMap<>();
    private static final HashMap<String, Integer> storedScaleDecayFactors = new HashMap<>();

    private static final HashMap<String, Float> storedAlphaLoops = new HashMap<>();
    private static final HashMap<String, Integer> storedAlphaDecayFactors = new HashMap<>();
    private static final HashMap<String, Float> storedTextScaleHovered = new HashMap<>();

    private static final HashMap<String, Float> storedTextEnableLoops = new HashMap<>();
    private static final HashMap<String, Integer> storedTextEnableDecayFactors = new HashMap<>();

    private static final HashMap<String, Float> storedDisableSideColorModeLoops = new HashMap<>();
    private static final HashMap<String, Integer> storedDisableSideColorModeDecayFactors = new HashMap<>();

    private static final HashMap<String, Float> storedSideIconColorLoops = new HashMap<>();
    private static final HashMap<String, Integer> storedSideIconColorDecayFactors = new HashMap<>();

    private float moduleNameX;
    private float moduleNameXCentered;
    private float moduleNameXRight;
    private final boolean bigIcons = ClickGUI.instance.sideIconMode.getValue() != ClickGUI.SideIconMode.Dots && ClickGUI.instance.sideIconMode.getValue() != ClickGUI.SideIconMode.None;
    ...
    private float loopsAndShit(HashMap<String, Float> storage, HashMap<String, Integer> decay, float factor) {

        if (module.moduleToggleFlag) {
            storage.put(module.name, 0.0f);
            decay.put(module.name, 0);
            return 0.0f;
        }
        else {
            storage.putIfAbsent(module.name, 300.0f);
            decay.putIfAbsent(module.name, 169);
            float localLoops = storage.get(module.name);
            int decayFactor = decay.get(module.name);
            ...
            float output = localLoops;
            localLoops += (30 * factor) * Math.pow(0.97f , decayFactor);
            decayFactor += 1;
            storage.put(module.name, localLoops);
            decay.put(module.name, decayFactor);
            return output;
        }
    }
    ...
    public void render(int mouseX, int mouseY, float translateDelta, float partialTicks) {
        ...
        float loopsNum = loopsAndShit(storedLocalLoops, storedDecayFactors, ClickGUI.instance.enabledRectAnimationFactor.getValue());
        float loopsNumSide = loopsAndShit(storedLocalLoops, storedDecayFactors, ClickGUI.instance.enabledSideMoveFactor.getValue());
        float loopsNumScale = loopsAndShit(storedScaleLoops, storedScaleDecayFactors, ClickGUI.instance.enabledRectScaleFactor.getValue());
        float loopsNumAlpha = loopsAndShit(storedAlphaLoops, storedAlphaDecayFactors, ClickGUI.instance.enabledAllGlowAlphaFactor.getValue());
        float loopsNumText = loopsAndShit(storedTextEnableLoops, storedTextEnableDecayFactors, ClickGUI.instance.enabledTextSmoothFactor.getValue());
        float loopsNumSideColorMode = loopsAndShit(storedDisableSideColorModeLoops, storedDisableSideColorModeDecayFactors, ClickGUI.instance.enabledSideColorModeAnimationFactor.getValue());
        float loopsNumSideIconColor = loopsAndShit(storedSideIconColorLoops, storedSideIconColorDecayFactors, ClickGUI.instance.enabledSideIconColorSmoothFactor.getValue());
        module.moduleToggleFlag = false;
        ...
        renderHoverRect(module.name, mouseX, mouseY, x, y, x + width, y + height, 0.0f, 0.0f, true);
        ...
        float moduleCustomFontNameY = ((int) (y + height / 2 - font.getHeight() / 2f) + 2) + CustomFont.instance.textOffset.getValue();
        float currentTextWidth1 = FontManager.getWidth(module.name);
        if (CustomFont.instance.font.getValue() == CustomFont.FontMode.Minecraft) {
            ...
            if (CustomFont.instance.moduleTextPos.getValue() != CustomFont.TextPos.Left) {
                if (CustomFont.instance.moduleTextPos.getValue() == CustomFont.TextPos.Center) {
                    GL11.glTranslatef(((1.0f - ((ClickGUI.instance.moduleHoverTextScale.getValue() && ClickGUI.instance.moduleHoverStuff.getValue()) ? storedTextScaleHovered.get(module.name) : CustomFont.instance.textScale.getValue())) * currentTextWidth1) / 2.0f, 0.0f, 0.0f);
                }
                else if (CustomFont.instance.moduleTextPos.getValue() == CustomFont.TextPos.Right) {
                    GL11.glTranslatef(((1.0f - ((ClickGUI.instance.moduleHoverTextScale.getValue() && ClickGUI.instance.moduleHoverStuff.getValue()) ? storedTextScaleHovered.get(module.name) : CustomFont.instance.textScale.getValue())) * currentTextWidth1), 0.0f, 0.0f);
                }
            }
            ...
            if (CustomFont.instance.moduleTextPos.getValue() != CustomFont.TextPos.Left) {
                if (CustomFont.instance.moduleTextPos.getValue() == CustomFont.TextPos.Center) {
                    GL11.glTranslatef((((1.0f - ((ClickGUI.instance.moduleHoverTextScale.getValue() && ClickGUI.instance.moduleHoverStuff.getValue()) ? storedTextScaleHovered.get(module.name) : CustomFont.instance.textScale.getValue())) * currentTextWidth1) / 2.0f) * -1.0f, 0.0f, 0.0f);
                }
                else if (CustomFont.instance.moduleTextPos.getValue() == CustomFont.TextPos.Right) {
                    GL11.glTranslatef(((1.0f - ((ClickGUI.instance.moduleHoverTextScale.getValue() && ClickGUI.instance.moduleHoverStuff.getValue()) ? storedTextScaleHovered.get(module.name) : CustomFont.instance.textScale.getValue())) * currentTextWidth1) * -1.0f, 0.0f, 0.0f);
                }
            }

            if (ClickGUI.instance.moduleHoverTextScale.getValue() && ClickGUI.instance.moduleHoverStuff.getValue() && !isHovered(mouseX, mouseY, x, y, x + width, y + height)) {
                float hoverTextScaleLoops = storedTextScaleHovered.get(module.name);
            ...
            if (ClickGUI.instance.sideIconSide.getValue() == ClickGUI.SideIconSide.Right) {
                if (isExtended) {
                    if (ClickGUI.instance.sideIconMode.getValue() == ClickGUI.SideIconMode.Plus || ClickGUI.instance.sideIconMode.getValue() == ClickGUI.SideIconMode.Future) {
                        ...
                        FontManager.drawIconExtended((int)(theX), y + 5 + CustomFont.instance.textOffset.getValue().intValue(), sideIconColor);
                        ...
                        FontManager.drawIconExtended((int)(theX), y + 5 + (int)CustomFont.instance.textOffset.getValue().intValue(), sideIconColor);
                    ...
                    if (ClickGUI.instance.sideIconMode.getValue() == ClickGUI.SideIconMode.Future) {
                        ...
                        FontManager.drawIcon((int)(theX), y + 5 + CustomFont.instance.textOffset.getValue().intValue(), sideIconColor);
