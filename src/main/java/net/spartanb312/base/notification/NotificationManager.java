...
public class NotificationManager {
    ...
    public static void moduleToggle(Module module, String name, boolean toggled) {
        if (ChatSettings.INSTANCE.autoDeleteToggleMessages.getValue()) {
            if (ChatSettings.INSTANCE.toggleMessagesMarked.getValue()) {
                ...
                ChatUtil.sendRawNoSpamMessage(ChatUtil.effectString(ChatSettings.INSTANCE.moduleEffects) + name + "\u00a7r " + ChatUtil.SECTIONSIGN + " has been " + (toggled ? color("aEnabled") : color("cDisabled")) + color("r") + "!");
            }
        }
        else {
            if (ChatSettings.INSTANCE.toggleMessagesMarked.getValue()) {
                ChatUtil.printChatMessage(ChatUtil.effectString(ChatSettings.INSTANCE.moduleEffects) + name + "\u00a7r " + ChatUtil.SECTIONSIGN + " has been " + (toggled ? color("aEnabled") : color("cDisabled")) + color("r") + "!");
            } else {
                ChatUtil.printRawChatMessage(ChatUtil.effectString(ChatSettings.INSTANCE.moduleEffects) + name + "\u00a7r " + ChatUtil.SECTIONSIGN + " has been " + (toggled ? color("aEnabled") : color("cDisabled")) + color("r") + "!");
            }
        }
    }
