...
public class SpongeCrystal extends Module {
    ...
    Setting<Boolean> targetMonsters = setting("TargetMonsters", false).des("Target monsters").whenAtMode(page, Page.General);
    ...
    Setting<Color> solidColor = setting("SolidColor", new Color(new java.awt.Color(100, 61, 255, 50).getRGB())).whenAtMode(page, Page.Render);
    ...
    Setting<Color> linesColor = setting("LinesColor", new Color(new java.awt.Color(255, 255, 255, 120).getRGB())).whenAtMode(page, Page.Render);
    ...
    RepeatUnit updateCalc = new RepeatUnit(() -> targetUpdateDelay.getValue(), () -> {
        Pair<BlockPos, Entity> data = CrystalUtil.calcPlace(targetMonsters.getValue(), detectionRange.getValue(), range.getValue(), wallRange.getValue(),
    ...
    public void onUpdateWalkingPlayerPre(OnUpdateWalkingPlayerEvent.Pre event) {
        if (rotate.getValue() && toPlacePos != null && !(conserveSponges.getValue() && prevSpongePos != null && mc.world.getBlockState(prevSpongePos).getBlock() == Blocks.SPONGE)) {
            if (mc.world.getBlockState(BlockUtil.extrudeBlock(toPlacePos, EnumFacing.UP)).getBlock() == Blocks.WATER
                ...
                for (EnumFacing facing : EnumFacing.HORIZONTALS) {
                    ...
                    if (BlockUtil.isFacePlaceble(pos, EnumFacing.UP, true)) {
                        RotationManager.setYawAndPitchMotionEvent(event, pos, EnumFacing.UP);
                    }
                }
            }
        }
            ...
            for (EnumFacing facing : EnumFacing.HORIZONTALS) {
                ...
                if (BlockUtil.isFacePlaceble(pos, EnumFacing.UP, true)) {
                    ...
                    if (ItemUtils.isItemInHotbar(Item.getItemFromBlock(Blocks.SPONGE))) {
                        ...
                        ItemUtils.switchToSlot(ItemUtils.findItemInHotBar(Item.getItemFromBlock(Blocks.SPONGE)), false);
                    }

                    BlockUtil.placeBlock(pos, EnumFacing.UP, false, packetPlace.getValue(), false);
