...
public class EnderChestFarm extends Module {
    ...
    Setting<Boolean> packetMine = setting("PacketMine", true).des("Uses packets to mine instead of clicking on the block").when(() -> !(!clientSideRotate.getValue() && rotate.getValue()));
    ...
    Setting<me.afterdarkness.moloch.core.common.Color> solidColor = setting("SolidColor", new me.afterdarkness.moloch.core.common.Color(new Color(255, 255, 255, 19).getRGB())).whenTrue(renderSolid).whenTrue(render);
    ...
    Setting<me.afterdarkness.moloch.core.common.Color> linesColor = setting("LinesColor", new me.afterdarkness.moloch.core.common.Color(new Color(255, 255, 255, 101).getRGB())).whenTrue(renderLines).whenTrue(render);
        ...
        if (renderPos != null && !placeableSpots.isEmpty()) {
            if (BlockUtil.isBlockPlaceable(renderPos)) {
                SwapManager.swapInvoke(this.name, false, true, () -> {
                    ItemUtils.switchToSlot(ItemUtils.fastestMiningTool(Blocks.ENDER_CHEST), false);
    ...
    public void onUpdateWalkingPlayerPre(OnUpdateWalkingPlayerEvent.Pre event) {
        if (!clientSideRotate.getValue() && rotate.getValue() && !((!ItemUtils.isItemInHotbar(Item.getItemFromBlock(Blocks.ENDER_CHEST)) && swapMode.getValue() == SwapMode.HotbarOnly)
            ...
            if (!placeableSpots.isEmpty()) {
                ...
                if (!BlockUtil.isBlockPlaceable(renderPos)) {
                    if (timer.passed(placeDelay.getValue())) {
                        RotationManager.setYawAndPitchMotionEvent(event, placeableSpots.get(0).a, placeableSpots.get(0).b);
                    }
                }
            }
        }
    ...
    public void onUpdateWalkingPlayer(OnUpdateWalkingPlayerEvent event) {
        ...
        if (renderPos != null && !placeableSpots.isEmpty() && BlockUtil.isBlockPlaceable(renderPos)) {
            ...
            if (((System.currentTimeMillis() - placeTimeStamp <= 50) || (remainingTime <= 100 && remainingTime >= 0)) && !clientSideRotate.getValue()) {
                mc.player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, renderPos, BlockUtil.getVisibleBlockSide(new Vec3d(renderPos))));
    ...
    private void update(OnUpdateWalkingPlayerEvent event) {
        ...
        //place stuff
        BlockPos pos = EntityUtil.floorEntity(mc.player);
                ...
                if (timer.passed(placeDelay.getValue())) {
                    SwapManager.swapInvoke(this.name, false, true, () -> {
                        ...
                        if (swapMode.getValue() == SwapMode.HotbarOnly || (swapMode.getValue() == SwapMode.FromInventory && ItemUtils.isItemInHotbar(Item.getItemFromBlock(Blocks.ENDER_CHEST)))) {
                            ItemUtils.switchToSlot(ItemUtils.findBlockInHotBar(Blocks.ENDER_CHEST), false);
                        }
                        ...
                        BlockUtil.placeBlock(placeableSpots.get(0).a, placeableSpots.get(0).b, false, packetPlace.getValue(), swapMode.getValue() == SwapMode.Offhand);
                    });
