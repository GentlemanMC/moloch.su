...
public class PopRender extends Module {
    ...
    Setting<Float> moveSpeed = setting("MovementSpeed", 1.0f, 0.1f, 3.0f).when(() -> movement.getValue() != Movement.None).whenAtMode(page, Page.General);
    ...
    Setting<Color> selfPointColor = setting("SelfPointColor", new Color(new java.awt.Color(255, 255, 255, 120).getRGB())).whenTrue(points).whenTrue(self).whenAtMode(page, Page.Colors);
    Setting<Color> selfSolidColor = setting("SelfSolidColor", new Color(new java.awt.Color(255, 255, 255, 40).getRGB())).whenTrue(solid).whenTrue(self).whenAtMode(page, Page.Colors);
    Setting<Color> selfLinesColor = setting("SelfLinesColor", new Color(new java.awt.Color(255, 255, 255, 120).getRGB())).whenTrue(lines).whenTrue(self).whenAtMode(page, Page.Colors);
    Setting<Color> selfSkeletonColor = setting("SelfSkeletonColor", new Color(new java.awt.Color(255, 255, 255, 120).getRGB())).whenTrue(skeleton).whenTrue(self).whenAtMode(page, Page.Colors);

    Setting<Color> playerPointColor = setting("PlayerPointColor", new Color(new java.awt.Color(180, 255, 180, 120).getRGB())).whenTrue(points).whenAtMode(page, Page.Colors);
    Setting<Color> playerSolidColor = setting("PlayerSolidColor", new Color(new java.awt.Color(180, 255, 180, 40).getRGB())).whenTrue(solid).whenAtMode(page, Page.Colors);
    Setting<Color> playerLinesColor = setting("PlayerLinesColor", new Color(new java.awt.Color(180, 255, 180, 120).getRGB())).whenTrue(lines).whenAtMode(page, Page.Colors);
    Setting<Color> playerSkeletonColor = setting("PlayerSkeletonColor", new Color(new java.awt.Color(180, 255, 180, 120).getRGB())).whenTrue(skeleton).whenAtMode(page, Page.Colors);

    Setting<Color> friendPointColor = setting("FriendPointColor", new Color(new java.awt.Color(100, 255, 255, 120).getRGB())).whenTrue(points).whenAtMode(page, Page.Colors);
    Setting<Color> friendSolidColor = setting("FriendSolidColor", new Color(new java.awt.Color(100, 255, 255, 40).getRGB())).whenTrue(solid).whenAtMode(page, Page.Colors);
    Setting<Color> friendLinesColor = setting("FriendLinesColor", new Color(new java.awt.Color(100, 255, 255, 120).getRGB())).whenTrue(lines).whenAtMode(page, Page.Colors);
    Setting<Color> friendSkeletonColor = setting("FriendSkeletonColor", new Color(new java.awt.Color(100, 255, 255, 120).getRGB())).whenTrue(skeleton).whenAtMode(page, Page.Colors);

    Setting<Color> enemyPointColor = setting("EnemyPointColor", new Color(new java.awt.Color(255, 100, 100, 120).getRGB())).whenTrue(points).whenAtMode(page, Page.Colors);
    Setting<Color> enemySolidColor = setting("EnemySolidColor", new Color(new java.awt.Color(255, 100, 100, 40).getRGB())).whenTrue(solid).whenAtMode(page, Page.Colors);
    Setting<Color> enemyLinesColor = setting("EnemyLinesColor", new Color(new java.awt.Color(255, 100, 100, 120).getRGB())).whenTrue(lines).whenAtMode(page, Page.Colors);
    Setting<Color> enemySkeletonColor = setting("EnemySkeletonColor", new Color(new java.awt.Color(255, 100, 100, 120).getRGB())).whenTrue(skeleton).whenAtMode(page, Page.Colors);
    ...
    public void onRenderWorld(RenderEvent event) {
        ...
        if (multiRender.getValue()) {
            for (Map.Entry<EntityOtherPlayerMP, Float> entry : new HashMap<>(popMapMulti).entrySet()) {
                ...
                popMapMulti.put(entry.getKey(), entry.getValue() - ((passedms < 1000 ? passedms : 0.0f) * (fadeSpeed.getValue() / 5.0f)));

                if (movement.getValue() != Movement.None) {
                    popMoveMapMulti.put(entry.getKey(), (popMoveMapMulti.get(entry.getKey()) == null ? 0.0f : popMoveMapMulti.get(entry.getKey())) + ((passedms < 1000 ? passedms : 0.0f) * (moveSpeed.getValue() / 1400.0f)));
                }
            }
        }
        else {
            for (Map.Entry<Integer, Pair<EntityOtherPlayerMP, Float>> entry : new HashMap<>(popMapSingle).entrySet()) {
                ...
                popMapSingle.put(entry.getKey(), new Pair<>(entry.getValue().a, entry.getValue().b - ((passedms < 1000 ? passedms : 0.0f) * (fadeSpeed.getValue() / 5.0f))));

                if (movement.getValue() != Movement.None) {
                    popMoveMapSingle.put(entry.getKey(), (popMoveMapSingle.get(entry.getKey()) == null ? 0.0f : popMoveMapSingle.get(entry.getKey())) + ((passedms < 1000 ? passedms : 0.0f) * (moveSpeed.getValue() / 1400.0f)));
                }
            }
        }
    }
    ...
    private void render(EntityOtherPlayerMP entityPlayer, float limbSwing, float limbSwingAmount, float swingProgress, float alphaFactor, float moveFactor) {
        ...
        if (RenderHelper.isInViewFrustrum(tempPlayer)) {
            ...
            if (skeleton.getValue()) {
                ...
                SpartanTessellator.drawSkeleton(entityPlayer, limbRotations.getValue() ? SpartanTessellator.getRotationsFromModel(model) : rotations, skeletonLinesWidth.getValue(), skeletonFadeLimbs.getValue(), false, -1, -1,
                        new java.awt.Color(ColorUtil.getRed(color), ColorUtil.getGreen(color), ColorUtil.getBlue(color), (int)(ColorUtil.getAlpha(color) * alphaFactor / 300.0f)).getRGB());
