...
import me.afterdarkness.moloch.event.events.render.ItemModelEvent;
...
public class HeldModelTweaks extends Module {
    ...
    public Setting<Boolean> render3dOnTop = setting("Render3dOnTop", false).des("Renders Render3d stuff (i.e. holerender) on top of held model");
    ...
    Setting<Float> hitTransformBlendSpeed = setting("HitTransformBlendSpeed", 1.0f, 0.3f, 2.0f).des("Speed of blending normal transformations to hit transformations").whenAtMode(page, Page.Animations);
    public Setting<Float> minHitProgressReset = setting("MinHitProgressReset", 0.5f, 0.0f, 1.0f).des("Hit progress to be able to reset animation if one is already in progress").whenAtMode(page, Page.Animations);
    //See MixinEntityLivingBase
    ...
    Setting<Boolean> swordReverseSwingProgress = setting("SwordReverseSwingProgress", false).des("Make sword swing go backwards").whenTrue(swordModifyDiff).whenTrue(hitModify).whenAtMode(page, Page.Animations);
    ...
    //See MixinItemRenderer
    Setting<Boolean> equipProgressKeep = setting("EquipProgressKeep", false).des("Don't reset equip progress on swinging item").whenTrue(hitModify).whenAtMode(page, Page.Animations);
    ...
    private float swingAnimationBlendDelta = 0.0f;
    private final net.spartanb312.base.utils.Timer swingBlendTimer = new net.spartanb312.base.utils.Timer();
    ...
    public void onRenderTick() {
        if (hitModify.getValue()) {
            int passedms = (int) swingBlendTimer.hasPassed();
            swingBlendTimer.reset();

            if (passedms < 1000) {
                swingAnimationBlendDelta = MathUtilFuckYou.clamp(swingAnimationBlendDelta + ((isSwingInProgress ? 1.0f : -1.0f) * passedms * hitTransformBlendSpeed.getValue()), 0.0f, 300.0f);
            }
        }
    }
    ...
    public void modifyModelTransMatrix(ItemModelEvent.Normal event) {
        ...
        if (hitModify.getValue() && (isSwingInProgress || swingAnimationBlendDelta > 0.0f) && mc.player.swingingHand == event.hand) {
            ...
            switch (shouldSword ? swingSwordRotateAxis.getValue() : swingRotateAxis.getValue()) {
                case X: {
                    GL11.glRotatef(e1 * -(shouldSword ? swingSwordDepth.getValue() : swingDepth.getValue()), (shouldSword ? swordReverseSwingProgress.getValue() : reverseSwingProgress.getValue()) ? -1.0f : 1.0f, 0.0f, 0.0f);
                    break;
                }

                case Y: {
                    GL11.glRotatef(e1 * -(shouldSword ? swingSwordDepth.getValue() : swingDepth.getValue()), 0.0f, (shouldSword ? swordReverseSwingProgress.getValue() : reverseSwingProgress.getValue()) ? -1.0f : 1.0f, 0.0f);
                    break;
                }

                case Z: {
                    GL11.glRotatef(e1 * -(shouldSword ? swingSwordDepth.getValue() : swingDepth.getValue()), 0.0f, 0.0f, (shouldSword ? swordReverseSwingProgress.getValue() : reverseSwingProgress.getValue()) ? -1.0f : 1.0f);
                    break;
                }
            }
            ...
            if (shouldSword) {
                if (event.hand == EnumHand.MAIN_HAND) {
                    GL11.glRotatef(swordHitRotateX.getValue() * (swingAnimationBlendDelta / 300.0f), 1.0f, 0.0f, 0.0f);
                    GL11.glRotatef(swordHitRotateY.getValue() * (swingAnimationBlendDelta / 300.0f), 0.0f, 1.0f, 0.0f);
                    GL11.glRotatef(swordHitRotateZ.getValue() * (swingAnimationBlendDelta / 300.0f), 0.0f, 0.0f, 1.0f);
                }
                ...
                if (event.hand == EnumHand.MAIN_HAND) {
                    GL11.glRotatef(hitRotateX.getValue() * (swingAnimationBlendDelta / 300.0f), 1.0f, 0.0f, 0.0f);
                    GL11.glRotatef(hitRotateY.getValue() * (swingAnimationBlendDelta / 300.0f), 0.0f, 1.0f, 0.0f);
                    GL11.glRotatef(hitRotateZ.getValue() * (swingAnimationBlendDelta / 300.0f), 0.0f, 0.0f, 1.0f);
                }
    ...
    public void modifyModelTransMatrixPre(ItemModelEvent.Pre event) {
        if (hitModify.getValue() && (isSwingInProgress || swingAnimationBlendDelta > 0.0f) && mc.player.swingingHand == event.hand) {
            if (swordModifyDiff.getValue() && (mc.player.swingingHand == EnumHand.MAIN_HAND ? mc.player.getHeldItemMainhand().getItem() instanceof ItemSword : mc.player.getHeldItemOffhand().getItem() instanceof ItemSword)) {
                if (event.hand == EnumHand.MAIN_HAND) {
                    GL11.glTranslatef(swordHitX.getValue() * (swingAnimationBlendDelta / 300.0f), swordHitY.getValue() * (swingAnimationBlendDelta / 300.0f), -swordHitZ.getValue() * (swingAnimationBlendDelta / 300.0f));
                }
                ...
                if (event.hand == EnumHand.MAIN_HAND) {
                    GL11.glTranslatef(hitX.getValue() * (swingAnimationBlendDelta / 300.0f), hitY.getValue() * (swingAnimationBlendDelta / 300.0f), -hitZ.getValue() * (swingAnimationBlendDelta / 300.0f));
                }
