...
public class ArmorDisplay extends HUDModule {
    ...
    Setting<Integer> rectHeight = setting("RectHeight", 7, 1, 20).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Boolean> roundedRect = setting("RoundedRect", false).des("Rounded rect for non image render mode").when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Float> roundedRectRadius = setting("RoundedRectRadius", 0.6f, 0.0f, 1.0f).des("Radius of rounded rect").whenTrue(roundedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Boolean> roundedRectTopRight = setting("RoundedRectTopRight", true).des("Rounded corner for rounded rect top right").whenTrue(roundedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Boolean> roundedRectTopLeft = setting("RoundedRectTopLeft", true).des("Rounded corner for rounded rect top left").whenTrue(roundedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Boolean> roundedRectDownRight = setting("RoundedRectDownRight", true).des("Rounded corner for rounded rect bottom right").whenTrue(roundedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Boolean> roundedRectDownLeft = setting("RoundedRectDownLeft", true).des("Rounded corner for rounded rect bottom left").whenTrue(roundedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Boolean> borderedRect = setting("BorderedRect", true).des("Bordered rect for non image render mode").when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Float> borderedRectOffset = setting("BorderedRectOffset", 2.0f, 0.0f, 5.0f).des("Bordered rect outline offset").whenTrue(borderedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    Setting<Float> borderedRectWidth = setting("BorderedRectWidth", 1.2f, 1.0f, 2.0f).des("Bordered rect outline width").whenTrue(borderedRect).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.ArmorRender);
    ...
    Setting<Integer> shadowOffsetX = setting("ShadowOffsetX", 2, -50, 50).when(() -> shadowIndependent.getValue() || horizontal.getValue()).whenTrue(shadow).whenAtMode(page, Page.Shadow);
    Setting<Integer> shadowOffsetY = setting("ShadowOffsetY", 2, -50, 50).when(() -> shadowIndependent.getValue() || !horizontal.getValue()).whenTrue(shadow).whenAtMode(page, Page.Shadow);
    ...
    Setting<Color> rectColorElytra = setting("RectColorElytra", new Color(new java.awt.Color(150, 100, 150, 255).getRGB())).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorElytra = setting("DMGColorElytra", new Color(new java.awt.Color(50, 255, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorElytra2 = setting("DMGColorElytra2", new Color(new java.awt.Color(255, 50, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenTrue(damageColorShift).whenAtMode(page, Page.Colors);
    Setting<Color> rectColorDiamond = setting("RectColorDiamond", new Color(new java.awt.Color(53, 200, 200, 255).getRGB())).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorDiamond = setting("DMGColorDiamond", new Color(new java.awt.Color(50, 255, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorDiamond2 = setting("DMGColorDiamond2", new Color(new java.awt.Color(255, 50, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenTrue(damageColorShift).whenAtMode(page, Page.Colors);
    Setting<Color> rectColorIron = setting("RectColorIron", new Color(new java.awt.Color(200, 200, 200, 255).getRGB())).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorIron = setting("DMGColorIron", new Color(new java.awt.Color(50, 255, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorIron2 = setting("DMGColorIron2", new Color(new java.awt.Color(255, 50, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenTrue(damageColorShift).whenAtMode(page, Page.Colors);
    Setting<Color> rectColorGold = setting("RectColorGold", new Color(new java.awt.Color(200, 200, 50, 255).getRGB())).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorGold = setting("DMGColorGold", new Color(new java.awt.Color(50, 255, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorGold2 = setting("DMGColorGold2", new Color(new java.awt.Color(255, 50, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenTrue(damageColorShift).whenAtMode(page, Page.Colors);
    Setting<Color> rectColorChainmail = setting("RectColorChainmail", new Color(new java.awt.Color(150, 150, 150, 255).getRGB())).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorChainmail = setting("DMGColorChainmail", new Color(new java.awt.Color(50, 255, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorChainmail2 = setting("DMGColorChainmail2", new Color(new java.awt.Color(255, 50, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenTrue(damageColorShift).whenAtMode(page, Page.Colors);
    Setting<Color> rectColorLeather = setting("RectColorLeather", new Color(new java.awt.Color(110, 75, 0, 255).getRGB())).when(() -> renderMode.getValue() != RenderMode.Image).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorLeather = setting("DMGColorLeather", new Color(new java.awt.Color(50, 255, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenAtMode(page, Page.Colors);
    Setting<Color> DMGColorLeather2 = setting("DMGColorLeather2", new Color(new java.awt.Color(255, 50, 50, 255).getRGB())).when(() -> damageBar.getValue() || DMGPercentText.getValue()).whenTrue(damageColorShift).whenAtMode(page, Page.Colors);
    Setting<Color> rectColorDMGBarBG = setting("RectColorDMGBarBG", new Color(new java.awt.Color(20, 20, 20, 255).getRGB())).whenTrue(damageBar).whenAtMode(page, Page.Colors);
            ...
            public void onUpdate(ScaledResolution resolution, int mouseX, int mouseY) {
                ...
                for (ItemStack armorItem : mc.player.inventory.armorInventory) {
                    if (armorItem.getItem() != Items.AIR) {
                        ...
                        if (renderMode.getValue() == RenderMode.Simplified) {
    ...
    public void onHUDRender(ScaledResolution resolution) {
        if (renderMode.getValue() == RenderMode.Image || armorCount.getValue()) {
            int x1 = horizontal.getValue() ? x + (separationDist.getValue() * 3) : x;
            int y1 = horizontal.getValue() ? y - (shiftInWater.getValue() && mc.player.isInWater() && mc.playerController.gameIsSurvivalOrAdventure() ? 10 : 0) : y + (separationDist.getValue() * 3);
            width = horizontal.getValue() ? separationDist.getValue() * 4 : rectsWidth.getValue() + 5;
            height = horizontal.getValue() ? (renderMode.getValue() == RenderMode.Simplified ? rectHeight.getValue() + 5 : 20) : separationDist.getValue() * 4;
            ...
            for (ItemStack armorItem : mc.player.inventory.armorInventory) {
                if (armorItem.getItem() != Items.AIR) {
                    ...
                    if (renderMode.getValue() == RenderMode.Image) {
                    ...
                    if (armorCount.getValue() && armorNum > 1) {
                        ...
                        FontManager.drawHUD(ItemUtils.getItemCount(armorItem.getItem()) + 1 + "", x1 + armorCountOffsetX.getValue(), y1 + armorCountOffsetY.getValue(), armorCountShadow.getValue(), new java.awt.Color(255, 255, 255, 255).getRGB());
                    ...
                    if (DMGPercentText.getValue()) {
                        ...
                        FontManager.drawHUD((int)(ItemUtils.getItemDMG(armorItem) * 100) + "", x1 + DMGPercentOffsetX.getValue(), y1 + DMGPercentOffsetY.getValue(), DMGPercentShadow.getValue(), damageColorShift.getValue() ? ColorUtil.colorShift(getDMGColor2(armorItem.getItem()), getDMGColor(armorItem.getItem()), ItemUtils.getItemDMG(armorItem)) : getDMGColor(armorItem.getItem()));
    ...
    private void renderDMGBar(AsyncRenderer asyncRenderer, int x, int y, ItemStack itemStack) {
        int dmgBarColor = getDMGColor(itemStack.getItem());

        if (damageColorShift.getValue()) {
            dmgBarColor = ColorUtil.colorShift(getDMGColor2(itemStack.getItem()), getDMGColor(itemStack.getItem()), ItemUtils.getItemDMG(itemStack));
        }

        if (borderedRectDMGBar.getValue()) {
            if (roundedRectDMGBar.getValue()) {
                asyncRenderer.drawAsyncRoundedRectOutline(x, y, roundedRectRadiusDMGBar.getValue(), x + rectsWidth.getValue(), y + DMGBarHeight.getValue(), borderedRectWidthDMGBar.getValue(), roundedRectTopRight.getValue(), roundedRectTopLeft.getValue(), roundedRectDownRight.getValue(), roundedRectDownLeft.getValue(), dmgBarColor);
                asyncRenderer.drawAsyncRoundedRect(x + (borderedRectOffsetDMGBar.getValue() / 2.0f), y + (borderedRectOffsetDMGBar.getValue() / 2.0f), roundedRectRadiusDMGBar.getValue(), x + (ItemUtils.getItemDMG(itemStack) * rectsWidth.getValue()) - (borderedRectOffsetDMGBar.getValue() / 2.0f), y + DMGBarHeight.getValue() - (borderedRectOffsetDMGBar.getValue() / 2.0f), roundedRectTopRight.getValue(), roundedRectTopLeft.getValue(), roundedRectDownRight.getValue(), roundedRectDownLeft.getValue(), dmgBarColor);
            }
            else {
                asyncRenderer.drawAsyncRectOutline(x, y, x + rectsWidth.getValue(), y + DMGBarHeight.getValue(), borderedRectWidthDMGBar.getValue(), dmgBarColor);
                asyncRenderer.drawAsyncRect(x + (borderedRectOffsetDMGBar.getValue() / 2.0f), y + (borderedRectOffsetDMGBar.getValue() / 2.0f), x + (ItemUtils.getItemDMG(itemStack) * rectsWidth.getValue()) - (borderedRectOffsetDMGBar.getValue() / 2.0f), y + DMGBarHeight.getValue() - (borderedRectOffsetDMGBar.getValue() / 2.0f), dmgBarColor);
            }
        }
        else {
            if (roundedRectDMGBar.getValue()) {
                asyncRenderer.drawAsyncRoundedRect(x, y, roundedRectRadiusDMGBar.getValue(), x + (ItemUtils.getItemDMG(itemStack) * rectsWidth.getValue()), y + DMGBarHeight.getValue(), roundedRectTopRight.getValue(), roundedRectTopLeft.getValue(), roundedRectDownRight.getValue(), roundedRectDownLeft.getValue(), dmgBarColor);
            }
            else {
                asyncRenderer.drawAsyncRect(x, y, x + (ItemUtils.getItemDMG(itemStack) * rectsWidth.getValue()), y + DMGBarHeight.getValue(), dmgBarColor);
            }
        }
    }
    ...
    private int getDMGColor(Item item) {
        if (item == Items.LEATHER_BOOTS || item == Items.LEATHER_LEGGINGS || item == Items.LEATHER_CHESTPLATE || item == Items.LEATHER_HELMET) {
            return DMGColorLeather.getValue().getColor();
        }

        if (item == Items.CHAINMAIL_BOOTS || item == Items.CHAINMAIL_LEGGINGS || item == Items.CHAINMAIL_CHESTPLATE || item == Items.CHAINMAIL_HELMET) {
            return DMGColorChainmail.getValue().getColor();
        }

        if (item == Items.GOLDEN_BOOTS || item == Items.GOLDEN_LEGGINGS || item == Items.GOLDEN_CHESTPLATE || item == Items.GOLDEN_HELMET) {
            return DMGColorGold.getValue().getColor();
        }

        if (item == Items.IRON_BOOTS || item == Items.IRON_LEGGINGS || item == Items.IRON_CHESTPLATE || item == Items.IRON_HELMET) {
            return DMGColorIron.getValue().getColor();
        }

        if (item == Items.DIAMOND_BOOTS || item == Items.DIAMOND_LEGGINGS || item == Items.DIAMOND_CHESTPLATE || item == Items.DIAMOND_HELMET) {
            return DMGColorDiamond.getValue().getColor();
        }

        if (item == Items.ELYTRA) {
            return DMGColorElytra.getValue().getColor();
        }

        return new java.awt.Color(0).getRGB();
    }

    private int getDMGColor2(Item item) {
        if (item == Items.LEATHER_BOOTS || item == Items.LEATHER_LEGGINGS || item == Items.LEATHER_CHESTPLATE || item == Items.LEATHER_HELMET) {
            return DMGColorLeather2.getValue().getColor();
        }

        if (item == Items.CHAINMAIL_BOOTS || item == Items.CHAINMAIL_LEGGINGS || item == Items.CHAINMAIL_CHESTPLATE || item == Items.CHAINMAIL_HELMET) {
            return DMGColorChainmail2.getValue().getColor();
        }

        if (item == Items.GOLDEN_BOOTS || item == Items.GOLDEN_LEGGINGS || item == Items.GOLDEN_CHESTPLATE || item == Items.GOLDEN_HELMET) {
            return DMGColorGold2.getValue().getColor();
        }

        if (item == Items.IRON_BOOTS || item == Items.IRON_LEGGINGS || item == Items.IRON_CHESTPLATE || item == Items.IRON_HELMET) {
            return DMGColorIron2.getValue().getColor();
        }

        if (item == Items.DIAMOND_BOOTS || item == Items.DIAMOND_LEGGINGS || item == Items.DIAMOND_CHESTPLATE || item == Items.DIAMOND_HELMET) {
            return DMGColorDiamond2.getValue().getColor();
        }

        if (item == Items.ELYTRA) {
            return DMGColorElytra2.getValue().getColor();
        }
