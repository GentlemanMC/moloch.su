...
public class WaterMark extends HUDModule {
    ...
    public WaterMark() {
        asyncRenderer = new AsyncRenderer() {
            ...
            public void onUpdate(ScaledResolution resolution, int mouseX, int mouseY) {
                drawAsyncString(GlobalManagers.INSTANCE.clientName.getValue() + " " + ChatUtil.SECTIONSIGN + "f" + (version.getValue() ? GlobalManagers.INSTANCE.clientVersion : ""), x, y, color(), watermarkShadow.getValue());
                width = FontManager.getWidthHUD(GlobalManagers.INSTANCE.clientName.getValue() + " " + ChatUtil.SECTIONSIGN + "f" + (version.getValue() ? GlobalManagers.INSTANCE.clientVersion : ""));
    ...
    public int color() {
        if (rainbow.getValue()) {
            Color lgbtqColor = new Color(ColorUtil.staticRainbow(rainbowSpeed.getValue(), rainbowSaturation.getValue(), rainbowBrightness.getValue()));
