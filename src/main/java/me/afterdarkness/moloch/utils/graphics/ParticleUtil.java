package me.afterdarkness.moloch.utils.graphics;
...
public class ParticleUtil
{
    ...
    public static final LockTask particlesClearTask = new LockTask(ParticleUtil::clearParticles);
    private static final Timer particlesTimer = new Timer();
    private static float alphaThreader = 0.0f;

    public static void render()
    {
        particlesClearTask.setLocked(false);

        int particleColor = Particles.INSTANCE.particleColor.getValue().getColor();
        ...
        for (Map.Entry<Integer, Vector2f> entry : new HashMap<>(particlesList).entrySet())
        {
            ...
            {
                particleColor = new Color(ColorUtil.rolledRainbow((int)(entry.getValue().x), Particles.INSTANCE.particleRainbowRollSize.getValue() / 10.0f, Particles.INSTANCE.particleColor.getValue().getRainbowSpeed() / 10.0f, Particles.INSTANCE.particleColor.getValue().getRainbowSaturation(), Particles.INSTANCE.particleColor.getValue().getRainbowBrightness())).getRGB();
            }

            if (Particles.INSTANCE.particleRollColor.getValue())
            {
                particleColor = ColorUtil.rolledColor(Particles.INSTANCE.particleRollColor1.getValue().getColor(), Particles.INSTANCE.particleRollColor2.getValue().getColor(), (int)entry.getValue().x, Particles.INSTANCE.particleRollColorSpeed.getValue(), Particles.INSTANCE.particleRollColorSize.getValue() / 10.0f);
            ...
            if (Particles.INSTANCE.particlesShape.getValue() == Particles.ParticlesShape.Circle)
            {
                GL11.glColor4f(ColorUtil.getRed(particleColor) / 255.0f, ColorUtil.getGreen(particleColor) / 255.0f , ColorUtil.getRed(particleColor) / 255.0f, alpha / 255.0f);
            ...
            else
            {
                ...
                if (Particles.INSTANCE.particlesShape.getValue() == Particles.ParticlesShape.Triangle)
                    RenderUtils2D.drawEquilateralTriangle(entry.getValue().x, entry.getValue().y, false, particlesSize.get(entry.getKey()), new Color(ColorUtil.getRed(particleColor), ColorUtil.getGreen(particleColor), ColorUtil.getBlue(particleColor), (int)alpha).getRGB());

                if (Particles.INSTANCE.particlesShape.getValue() == Particles.ParticlesShape.Square)
                    RenderUtils2D.drawRhombus(entry.getValue().x, entry.getValue().y, particlesSize.get(entry.getKey()), new Color(ColorUtil.getRed(particleColor), ColorUtil.getGreen(particleColor), ColorUtil.getBlue(particleColor), (int)alpha).getRGB());
    ...
    {
        int lineColor = Particles.INSTANCE.lineColor.getValue().getColor();
        ...
        for (Map.Entry<Integer, Vector2f> entry : new HashMap<>(particlesList).entrySet())
        {
            ...
            if (Particles.INSTANCE.restrictToAroundMouseLines.getValue() && getParticleDist(new Vector2f(Mouse.getEventX() * mc.currentScreen.width / mc.displayWidth, mc.currentScreen.height - Mouse.getEventY() * mc.currentScreen.height / mc.displayHeight - 1), entry.getValue()) > Particles.INSTANCE.restrictToAroundMouseLinesRange.getValue())
                continue;
            ...
            if (getParticleDist(particle.getValue(), entry.getValue()) <= Particles.INSTANCE.connectRange.getValue())
            {
                ...
                {
                    lineColor = new Color(ColorUtil.rolledRainbow((int)(entry.getValue().x), Particles.INSTANCE.lineRainbowRollSize.getValue() / 10.0f, Particles.INSTANCE.lineColor.getValue().getRainbowSpeed(), Particles.INSTANCE.lineColor.getValue().getRainbowSaturation(), Particles.INSTANCE.lineColor.getValue().getRainbowBrightness())).getRGB();
                }

                if (Particles.INSTANCE.lineRollColor.getValue())
                {
                    lineColor = ColorUtil.rolledColor(Particles.INSTANCE.lineRollColor1.getValue().getColor(), Particles.INSTANCE.lineRollColor2.getValue().getColor(), (int)entry.getValue().x, Particles.INSTANCE.lineRollColorSpeed.getValue(), Particles.INSTANCE.lineRollColorSize.getValue() / 10.0f);
                ...
                GL11.glColor4f(ColorUtil.getRed(lineColor) / 255.0f, ColorUtil.getGreen(lineColor) / 255.0f, ColorUtil.getBlue(lineColor) / 255.0f, (alpha / 255.0f) * alphaFactor * (Particles.INSTANCE.linesFadeIn.getValue() ? ((Particles.INSTANCE.connectRange.getValue() - getParticleDist(particle.getValue(), entry.getValue())) / (Particles.INSTANCE.connectRange.getValue() / Particles.INSTANCE.linesFadeInFactor.getValue())) : 1.0f));
